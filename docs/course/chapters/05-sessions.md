...
网页中的代码保留阅读这条主线所需的骨架，完整实现以右侧源码为准。

### 第一步：先看 Session 持久化了哪些事实

在 `entries.py` 中找到 `EntryBase`。每条对话记录都有自己的身份，并通过 `parent_id` 指向上一条记录：
...
`UserMessageEntry`、`AssistantMessageEntry`、`ToolResultEntry` 和 `CompactionEntry` 都沿用这个结构。因此
Session 内部保存的是一棵 Entry 树，当前对话只是从某个 leaf 回到根的一条路径。

接着切到 `metadata.py`，找到 `SessionMetadata`：
...
- `leaf_id` 选择 Session 自己的当前 Entry 路径；
- `parent_session_id` 和 `fork_point_entry_id` 让一个 Fork Session 引用父 Session 的历史前缀。

因此 Phi 中有两层树：Session 内部的 Entry 树，以及多个 Session 之间的 Fork 谱系。`lineage_matches_origin()`
把两者的组合限制为三种有效形状：普通 Session 没有父谱系，Fork 同时拥有父 Session 和精确分叉点，
Subagent 只记录委派来源而不继承父对话。

### 第二步：理解一次追加怎样成为“已提交历史”

打开 `storage.py`，先看 `SessionMetadataEnvelope` 在哪里被使用。它除了 metadata，还保存两个提交字段：
...
一次消息往返可能产生多条 Entry。`SessionStorage.append_entries()` 先检查调用者持有的 revision，再构造
新的提交信封：
...
继续进入 `_append_and_commit_sync()`，观察写入顺序：

```text
新 Entries 追加到 .jsonl journal，并 fsync
        ↓
metadata 写入临时文件，并 fsync
        ↓
os.replace 原子替换 metadata
```

真正的提交点是 metadata 中的 `committed_entry_count`。如果进程在 journal 写完、metadata 更新前崩溃，
重启后的 `_load_state_sync()` 只解析已经提交的前缀，并把尾部报告为诊断：
...
持久化的核心不变量是：metadata 指向的 leaf 必须属于已提交 Entry；journal 中尚未被 metadata 确认的
内容不能进入 Conversation View。

### 第三步：从磁盘恢复不可变的 SessionHandle
...
`load_state()` 负责磁盘格式与提交一致性；`_session_branch_points()` 和
`materialize_conversation()` 负责结构与对话语义。两层检查都通过后，Host 才得到新的 `SessionHandle`。

`SessionHandle` 是不可变游标，而不是一份可以原地修改的 Session 对象。每次成功追加都会返回带新
`leaf_id` 和新 `revision` 的 Handle；调用者需要用新值替换旧值。

### 第四步：沿 leaf 物化完整路径

从 `materialize_presentation()` 进入 `_materialize_path()`。这个函数先在当前 Session 内建立 Entry
索引，再从 `leaf_id` 沿 `parent_id` 逆向收集：
...
普通 Session 到本地根就结束。Fork 的本地路径会在 `fork_point_entry_id` 处离开当前 journal，此时函数
加载父 Session，并递归物化父历史：
...
最终返回 `父前缀 + 当前 Session 的本地后缀`。`seen_sessions` 和 `seen_entries` 分别检测 Session 谱系环
与 Entry 环；缺失的 parent 也会变成明确的恢复错误，而不会被静默截断。

### 第五步：看 Fork 如何复用历史而不复制历史
...
此时新 Fork 的 journal 仍然为空。它通过 `parent_session_id` 和 `fork_point_entry_id` 共享父历史，之后的
新消息才进入自己的 journal。父 Session 继续指向原 leaf，原来的后续记录也保持不变。
...
最后阅读 `_validate_path()`。普通 User 消息和不带工具调用的 Assistant 消息都可以形成完整边界；带
Tool Call 的 Assistant 消息必须与紧随其后的全部 Tool Result 组成不可分割单元：
...
因此不能把 leaf 切到一组 Tool Calls 的中间，也不能从只完成了一部分 Tool Results 的位置 Fork。否则
下一次 Model Request 会看到无法配对的工具消息。

`CompactionEntry` 本身也是合法的分支点。它已经记录了摘要与保留边界，从这里继续或 Fork 都能物化出
一份完整、有效的 Conversation View。
...
2. 为什么 journal 尾部已经写入磁盘的 Entry 仍可能不属于已提交 Session？
3. `revision` 怎样阻止旧 `SessionHandle` 覆盖更新后的状态？
4. 一个空 journal 的 Fork 怎样恢复父 Session 的历史前缀？
5. 为什么 Tool Call 与对应的全部 Tool Results 只能作为一个完整分支边界？
