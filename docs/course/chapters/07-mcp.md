...
你们在 Claude Code/Codex 里配置过 MCP server 吗?体验上和接下来要做的 `phi mcp add` 是一回事。
仓库自带一个零依赖的本地测试 server,不用装任何东西、不用联网:

```bash
uv run phi mcp add demo -- uv run python tests/mcp/stdio_fixture.py
...
```text
输入:"调用 demo 服务器的 echo 工具，内容是 hello class"
```

看 Tool Call 卡片上的名字——`mcp__demo__echo`,直接对应下面第二步要讲的 namespacing。有余力可以
再试 `/mcp`(连接汇总)和 `/mcp__demo__welcome`(MCP Prompt 变成的 slash command)。
