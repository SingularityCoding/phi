...
## 动手:连一个 MCP server,看 doctor 怎么报告

你们在 Claude Code/Codex 里配置过 MCP server 吗?体验上和接下来要做的 `phi mcp add` 是一回事。这
次直接连 MCP 官方仓库([modelcontextprotocol/servers](https://github.com/modelcontextprotocol/servers))
维护的参考实现之一——[`mcp-server-time`](https://github.com/modelcontextprotocol/servers/tree/main/src/time),
纯本地时区计算,不发任何网络请求,`uvx` 首次运行会自动拉取,几秒内完成:

```bash
uv run phi mcp add clock -- uvx mcp-server-time
uv run phi doctor --deep -v
```

应该能看到:

```text
workspace.mcp-configuration   PASS   1 enabled · 0 disabled
deep.mcp-servers              PASS   1 of 1 connected
                                     clock: 2 Tools
```

再进 TUI(`uv run phi`)跑一次真实调用:

```text
输入:"现在纽约几点？调用 clock 服务器的工具查一下"
```

看 Tool Call 卡片上的名字——`mcp__clock__get_current_time`,直接对应下面第二步要讲的
namespacing。有余力可以再试 `/mcp`(连接汇总)或者让模型用 `convert_time` 换算时区。
