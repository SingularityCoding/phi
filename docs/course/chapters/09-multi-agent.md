...
Claude Code 的 Task 工具就是"父派生子、检查结果、不完全对等"这个形状。在 TUI(`uv run phi`)里
试一次委派:

```text
输入:"请把下面两个总结分别委托给两个 subagent 并行完成：
1) 一句话总结 README.md 在讲什么；
2) 一句话总结 CONTEXT.md 在讲什么。
都完成后合并汇报给我。"
```

观察 Tool Call 视图里出现两次 `spawn_agent`;还可以追问模型"现在有几个子 agent 在跑",触发
`list_agents`。
